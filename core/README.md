...
Долгий обрыв/рестарт процесса это не покрывает (приёмное состояние link'а
теряется) — там сессия завершается. Управляемый клиент `pkg/bproxy` после такого
завершения (включая GOAWAY сервера) сообщает статус `reconnecting` и повторяет
hub-rendezvous с экспоненциальным backoff. Локальный SOCKS/HTTP listener теперь
живёт дольше отдельных mux-сессий и остаётся привязанным к тому же порту; во
время reconnect новые подключения получают временную ошибку, а после ASSIGN
автоматически используют новую сессию. Это позволяет Android TUN engine не
перезапускать вместе с транспортом.
...
- Авто-реконнект транспорта переживает короткие блипы прозрачно (см. выше), но
  не рестарт процесса и не долгий обрыв — там приёмное состояние link'а теряется
  и сессия завершается. Резюме сессии через хаб по grace-окну не делали (store
  хранит только пользователей и хабы, без lifecycle соединений).
