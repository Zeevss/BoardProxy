...
// z85Alphabet — 85 печатных ASCII-символов, отступающих от спецификации Z85
// (ZeroMQ RFC 32): без пяти символов, которые стандартно экранируются в
// HTML/XML (кавычки и обратный слэш были исключены изначально — не требует
// JSON-экранирования), плюс '<', '>' и '&' — живая доска Яндекса рендерит
// '<...>' как HTML-тег и молча вырезает всё до следующего '>' (а для
// незакрытого '<' — вообще всё до конца значения), а одиночный '&' заменяет
// на литеральную подстроку "&amp;". Оба искажают длину и содержимое кадра.
// Замена — ',', ';', '_', проверены живьём.
const z85Alphabet = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ.-:+=^!/*?,;_()[]{}@%$#"
