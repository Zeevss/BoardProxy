...
Статус: **редакция 3, согласовано, в работе**.
...
Ориентир по объёму: сейчас 9 627 строк Kotlin в 140 файлах. Ожидаемый результат — 5 000–5 500.
...
- **Rate limiting**: остаётся в памяти процесса — честно только на одной реплике (см. O4).
...
| 1 | Новый `V1` + переписанные репозитории на JDBC | Testcontainers-тест на каждый репозиторий |
...
- **R1. Даунтайм при смене протокола ноды.** Старый агент не умеет `Watch`. Варианты: синхронное
  обновление всех нод либо временное сохранение `Connect` как deprecated рядом с новыми RPC.
  Зависит от O3.
...
- **R3. Kotlin переизобретает Go.** `goString()` (ручная реализация `time.Duration.String()`), схема
...
- ~~**O1.** Перенос данных.~~ Решено: пишем `V1` с нуля.
- ~~**O2.** Незакоммиченные 43 файла.~~ Частично: фронтенд не учитываем. **Остаётся решить**, что
  делать с уже написанным бэкендом в рабочем дереве (`SubscriptionService*`, `UserDirectory`,
  `AppliedConfigView`, миграция V12) — докоммитить как базу или откатить перед этапом 0.
- **O3. Go node-agent.** Кто и когда обновляет его под новый proto; нужен ли период совместимости
  со старым `Connect`, или ноды обновляются синхронно с хабом.
- **O4. Многорепличность.** Хаб реально запускается в нескольких репликах или это задел? Влияет на
  rate limiting (§8.3).
