...
- `panel/` — React/TypeScript-панель управления сервером;
- `docker-compose.yml` — совместный запуск: наружу опубликован только Caddy
  панели, запросы `/api/*` проксируются во внутренний HTTP API core.
...
9. `watchLane` независимо возвращает страницу в пул. Потеря одного lane не
...
- `Dashboard` — агрегатные и per-board метрики;
